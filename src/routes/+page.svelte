<script lang="ts">
  ...
  onMount(async () => {
    ...
    universe.add_planet("sun", 0, 0, 0, 0, 0, 0, 1.989e30, 196e5);
...
<div>
  {#if output}
    <div style="position:relative; width:100vmin;height:100vmin">
      ...
      {/each}
			<button on:click={addMoon}>Add Moon</button>

      <div
        style={`position:absolute; border-radius:50%; transform:translate(-50%, -50%);top:50%; left:50%;width:${5}px;height:${5}px;background-color:black;`}
      />
      {#each output as planet}
        <div
